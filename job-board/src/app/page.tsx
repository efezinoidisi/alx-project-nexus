...
import { SAMPLE_JOBS } from '@/constants/data';
...
export default function Home() {
  ...
  const categories = SAMPLE_JOBS.flatMap((job) => job.category).filter(
  ...
  const jobs = SAMPLE_JOBS.filter((job) => {
    ...
    const jobTypes =
      filters.type.length > 0
        ? filters.type.includes(job.jobType.toLowerCase())
        : true;

    const experienceLevels =
      filters.level.length > 0
        ? filters.level.includes(job.experienceLevel.toLowerCase())
        : true;
